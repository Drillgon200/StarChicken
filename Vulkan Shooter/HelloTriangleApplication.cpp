...
//This class is pretty much just the vulkan tutorial website
...
struct Vertex {
	...
	static uint32_t size() {
		return sizeof(glm::vec3) + 4 + sizeof(glm::vec2);
	}
...
private:
	...
	VkFormat findDepthFormat() {
		return findSupportedFormat({VK_FORMAT_D24_UNORM_S8_UINT, VK_FORMAT_D32_SFLOAT, VK_FORMAT_D32_SFLOAT_S8_UINT}, VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT);
	}
	...
	void createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags props, VkImage& image, VkDeviceMemory& imageMemory) {
		createImage(width, height, mipLevels, VK_SAMPLE_COUNT_1_BIT, format, tiling, usage, props, image, imageMemory);
	}

	void createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags props, VkImage& image, VkDeviceMemory& imageMemory) {
	...
	void transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels) {
		...
		if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED && newLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
			...
			throw std::invalid_argument("Unsupported transition layouts!");
		}
	...
	void createBuffer(VkDeviceSize size, VkBufferUsageFlags flags, VkMemoryPropertyFlags props, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
	...
	void createVertexBuffer() {
		VkDeviceSize bufferSize = vertices.size() * Vertex::size();
		...
		for (uint32_t i = 0; i < vertices.size(); i++) {
			memcpy(&vertexData[i * Vertex::size()], &vertices[i].pos, 12);
			uint32_t color = static_cast<uint32_t>(glm::packUnorm4x8(vertices[i].color));
			memcpy(&vertexData[i * Vertex::size() + 12], &color, 4);
			memcpy(&vertexData[i * Vertex::size() + 16], &vertices[i].texCoord, 8);
		}
	...
	void createGraphicsPipeline() {
		std::vector<char> vertShaderCode = readFile("resources/shaders/triangle.vspv");
		std::vector<char> fragShaderCode = readFile("resources/shaders/triangle.fspv");
